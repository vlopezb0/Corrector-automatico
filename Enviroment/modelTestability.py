...
def Testability(axs,debug=0):
    '''Modelo para predecir la medida de Testability
    ...
    for train,test in skf.split(X,Y):
        ...
        # Predecimos Testability y lo guardamos en el DataFrame
        pred = list(clf.predict(X_test))

        data.loc[test,"Predict_Testability"] = pred
        ...
        display = RocCurveDisplay.from_estimator(
                ...
                y_test,
                ax=axs,
                color = (0.2,0.5+(1-auc)/2,0.5+(1-auc)/2),
                label="_no",
                alpha = 1/510
            )
    ...
    data_predict=pd.DataFrame({"Nombre" : data["Nombre"],
                                "Predict_Testability" : data["Predict_Testability"]})
