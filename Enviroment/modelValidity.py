...
def Validity(axs,dp=None,debug=0):
    '''Modelo para predecir la medida de Validity

    Parametros:
        -axs : Subclase de la figura donde dibujar las curvas ROC.
        -dp : DataFrame con las predicciones de Well_Formed
    ...
    if dp is None:
        X = data[["WellFormed","Recursion Chat"]]
    else:
        # Pasamos datos inferidos
        data = pd.merge(data,dp,on="Nombre")
        X = data[["Predict_Well_Formed","Recursion Chat"]]

    Y = data["Validity"]

    # Creamos columnas en el DataFrame para los resultados
    data["Predict_Validity"] = -1
    ...
    for train,test in skf.split(X,Y):
        ...
        # Predecimos Validity y lo guardamos en el DataFrame
        pred = list(clf.predict(X_test))

        data.loc[test,"Predict_Validity"] = pred
    ...
    data_predict=pd.DataFrame({"Nombre" : data["Nombre"],
                                "Predict_Validity" : data["Predict_Validity"]})
